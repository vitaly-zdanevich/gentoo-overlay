...
SRC_URI="
	...
	x86? (
		images? (
			qr? (
				https://github.com/vitaly-zdanevich/youta/releases/download/v${PV}/${MY_P}-linux-i686.tar.gz
			)
			!qr? (
				https://github.com/vitaly-zdanevich/youta/releases/download/v${PV}/${MY_P}-linux-i686-no-qr.tar.gz
			)
		)
		!images? (
			qr? (
				https://github.com/vitaly-zdanevich/youta/releases/download/v${PV}/${MY_P}-linux-i686-text.tar.gz
			)
			!qr? (
				https://github.com/vitaly-zdanevich/youta/releases/download/v${PV}/${MY_P}-linux-i686-text-no-qr.tar.gz
			)
		)
	)
"
...
IUSE="+alsa cpu_flags_x86_sse2 +images jack pipewire pulseaudio +qr"
REQUIRED_USE="
	elibc_glibc
...
src_unpack() {
	...
	mv "${WORKDIR}/${MY_P}-linux-${release_arch}${archive_suffix}" "${S}" ||
